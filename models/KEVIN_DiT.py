...
class SwiGLU(nn.Module):
    def __init__(self, d_model, expansion=3, bias=False):
        super().__init__()

        self.gate = nn.Linear(d_model, expansion * d_model, bias=bias)
        self.up = nn.Linear(d_model, expansion * d_model, bias=bias)
        self.down = nn.Linear(expansion * d_model, d_model, bias=bias)

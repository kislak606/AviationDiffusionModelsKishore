...
# ─────────────────────────────────────────────────────────────────────────────
# Sinusoidal embedding — encodes diffusion time t into a vector
# (kept for AdaLN conditioning — RoPE replaces positional/trel embeddings only)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# RoPE — Rotary Position Embedding (Option A: token index positions)
# ─────────────────────────────────────────────────────────────────────────────
class RotaryEmbedding(nn.Module):
    def __init__(self, d_head):
        """
        d_head: dimension per attention head (d_model // n_heads)
        Precomputes frequency bands — not learned, just constants.
        """
        super().__init__()
        half  = d_head // 2
    ...
    def get_rotations(self, positions):
        """
        positions: (T,) token indices — 0, 1, 2, ..., 85
        returns:   cos (T, half), sin (T, half)
        """
        angles = positions.unsqueeze(-1) * self.freqs   # (T, half)
    ...
    def rotate(self, x, cos, sin):
        ...
        # (T, half) → (1, T, 1, half) for broadcasting
...
# ─────────────────────────────────────────────────────────────────────────────
# RoPE-aware self-attention — replaces nn.MultiheadAttention
# ─────────────────────────────────────────────────────────────────────────────
class RoPEAttention(nn.Module):
    ...
    def forward(self, x, positions, mask=None):
        """
        x:         (B, T, d_model)
        positions: (T,) token indices 0..85
        ...
        # Apply RoPE rotation to Q and K using token index positions
        cos, sin = self.rope.get_rotations(positions)
        Q = self.rope.rotate(Q, cos, sin)
        K = self.rope.rotate(K, cos, sin)

        # Reshape for attention: (B, n_heads, T, d_head)
        Q = Q.permute(0, 2, 1, 3)
        K = K.permute(0, 2, 1, 3)
        V = V.permute(0, 2, 1, 3)
...
# ─────────────────────────────────────────────────────────────────────────────
# AdaLN block — same as original but uses RoPEAttention
# ─────────────────────────────────────────────────────────────────────────────
class AdaLNBlock(nn.Module):
    def __init__(self, d_model, n_heads, dropout=0.1):
        ...
        # CHANGED: RoPEAttention instead of nn.MultiheadAttention
    ...
    def forward(self, x, cond, positions, mask=None):
        """
        x:         (B, T, d_model)
        cond:      (B, d_model) — AdaLN conditioning from diffusion time
        positions: (T,) token indices — passed to RoPEAttention
        mask:      (T, T) bool attention mask
        """
...
# ─────────────────────────────────────────────────────────────────────────────
# Full DiT with RoPE Option A (token index positions)
# ─────────────────────────────────────────────────────────────────────────────
class TrajectoryDiT(nn.Module):
    def __init__(self,
        ...
        seq_len      = obs_len + fut_len   # 86

        # Input projection: 6 features → d_model
        self.input_proj = nn.Linear(in_dim, d_model)

        # REMOVED: pos_emb (nn.Embedding) — RoPE handles position
        # REMOVED: trel_mlp — RoPE on token indices doesn't use t_rel
        # KEPT: tflow_mlp for AdaLN conditioning on diffusion/flow time
    ...
    def forward(self, x_obs, x_t, t, t_rel):
        ...
        t_rel:  (B, 86)     relative timestamps — NOT used by RoPE-A
                             (kept in signature for API compatibility with train.py)
        ...
        # Step 2: Option A positions — simple token indices 0..85
        # RoPE rotation angles are just the token index
        positions = torch.arange(
            self.obs_len + self.fut_len, device=device
        ).float()   # (86,)
